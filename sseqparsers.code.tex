...
\prg_new_conditional:Npnn \sseq_if_out_of_bounds:n #1 { T ,  F , TF } {
    ...
    \sseq@tempiftrue
    \ifnum\xcoord<\sseq@xmaxpp\relax\ifnum\xcoord>\sseq@xminmm\relax\ifnum\ycoord<\sseq@ymaxpp\relax\ifnum\ycoord>\sseq@yminmm\relax
        \sseq@tempiffalse
    \fi\fi\fi\fi
    \ifsseq@tempif
        \prg_return_true:
...
\sseq@DeclareDocumentCommand \Do { mm } {
    \def\iteration{0}
    \prg_replicate:nn {#1} {
        \edef\iteration{\the\numexpr\iteration+1}
        #2
...
\sseq@DeclareDocumentCommand \DoUntilOutOfBounds { m } {
    \sseq_if_out_of_bounds:nTF{\lastclass0}{
    ...
    \bool_until_do:Nn \l_tmpa_bool {
        \edef\iteration{\the\numexpr\iteration+1}
        #1
        \sseq_if_out_of_bounds:nT{\lastclass0}{
            \bool_set_true:N \l_tmpa_bool
        }
    }
}

\sseq@DeclareDocumentCommand \DoUntilOutOfBoundsThenNMore { m m } {
    \DoUntilOutOfBounds { #2 }
    \prg_replicate:nn {#1} {
        \edef\iteration{\the\numexpr\iteration+1}
        #2
    }
