...
\def\sseq@drawxaxis{
    ...
    \pgfpathmoveto{\pgfpointxy{\sseq@xmax+\sseq@xoffset}{\sseq@ymin+\sseq@yoffset}}
    \egroup
%
    \bgroup
    \pgftransformxshift{-\sseq@xaxis@start@extend}\pgftransformxshift{-\sseq@yaxisgap}
    \pgftransformyshift{-\sseq@xaxisgap}
    \pgfpathlineto{\pgfpointxy{\sseq@xmin+\sseq@xoffset}{\sseq@ymin+\sseq@yoffset}}
    \egroup
    \pgfusepath{stroke}
}
\def\sseq@drawyaxis{
    ...
    \pgfpathmoveto{\pgfpointxy{\sseq@xmin+\sseq@xoffset}{\sseq@ymax+\sseq@yoffset}}
    \egroup
    \bgroup
    \pgftransformyshift{-\sseq@xaxisgap}
...
\def\sseq@drawxticks{%
    ...
    \loop
        \bgroup
        \sseq@eval{\@nx\pgftransformshift{\@nx\pgfpointxy{\the\sseq@tempx+\sseq@xoffset}{\the\sseq@tempy+\sseq@yoffset}}}%
...
\def\sseq@setupclip{
    ...
    \ifsseq@clip
        \ifx\sseq@customclip\pgfutil@empty
            ...
            \pgfpathmoveto{\pgfpointxy{\sseq@xmin+\sseq@xoffset}{\sseq@ymin+\sseq@yoffset}}
            \egroup
            \bgroup
            \xmin\ymax%\sseq@globalscaletransform
            \pgfpathlineto{\pgfpointxy{\sseq@xmin+\sseq@xoffset}{\sseq@ymax+\sseq@yoffset}}
            \egroup
            \bgroup
            \xmax\ymax%\sseq@globalscaletransform
            \pgfpathlineto{\pgfpointxy{\sseq@xmax+\sseq@xoffset}{\sseq@ymax+\sseq@yoffset}}
            \egroup
            \bgroup
            \xmax\ymin%\sseq@globalscaletransform
            \pgfpathlineto{\pgfpointxy{\sseq@xmax+\sseq@xoffset}{\sseq@ymin+\sseq@yoffset}}
            \egroup
            \egroup
            \pgfpathclose
...
\def\sseq@grid@chess{
 ...
 \pgfscope
 \sseq@useclip
 \pgf@process{\pgfpointxy{\sseq@xgridstep}{0}}
 \pgf@xa=\pgf@x
 \pgf@process{\pgfpointxy{0}{\sseq@ygridstep}}
 \pgf@ya=\pgf@y
 \pgfmathparse{1cm/\pgf@xa}
 \pgftransformxscale{\pgfmathresult}
 \pgfmathparse{1cm/\pgf@ya}
 \pgftransformyscale{\pgfmathresult}
 \pgflowlevelsynccm
 \pgfsetlinewidth{1cm}
 \pgfsetdash{{1cm}{1cm}}{1cm}
 \pgfsetcolor{\sseq@gridcolor}
 \sseq@tempx=\numexpr\sseq@intdivceiling{\sseq@xmin}{2}*2\relax
 \sseq@tempy=\numexpr\sseq@intdivceiling{\sseq@ymin}{2}*2\relax
 ...
 \pgfpathgrid[stepx=2cm,stepy=2cm]
    ...
    { \pgfpointxy { \the\sseq@tempxb + 0.499 + \sseq@xoffset + 1 }{ \the\sseq@tempyb + 0.499 + \sseq@yoffset + 1 } }
 \pgfusepath{stroke}
 \endpgfscope
 \egroup
}
\def\sseq@grid@crossword{
 ...
 \pgfpathgrid[stepx=\pgf@xa,stepy=\pgf@ya]
    { \pgfpointxy
        { \xmin - \sseq@tempx + \sseq@xoffset }
        { \ymin - \sseq@tempy + \sseq@yoffset} }
    { \pgfpointxy
        { \xmax + \sseq@tempxb - 0.01 + \sseq@xoffset }
        { \ymax + \sseq@tempyb - 0.01 + \sseq@yoffset }
    }
 \pgfusepath{stroke}
 \endpgfscope
 \egroup
...
\def\sseq@grid@go{
 ...
 \sseq@tempx=\numexpr \dimexpr(\sseq@yaxisgap-\sseq@yclip@axisgap)\relax/\pgf@xa+1\relax
 \sseq@tempy=\numexpr \dimexpr(\sseq@xaxisgap-\sseq@xclip@axisgap)\relax/\pgf@ya+1\relax
 \sseq@tempxb=\numexpr \dimexpr\sseq@xaxis@end@extend\relax/\pgf@xa\relax
 \sseq@tempyb=\numexpr \dimexpr\sseq@yaxis@end@extend\relax/\pgf@ya\relax
...
\def\sseq@offset#1#2{
    \sseq@eval{\@nx\pgftransformshift{
        \@nx\pgfpointxy
            { 0\csname sseq@\sseq@classpattern xoffset#1/#2\endcsname }
            { 0\csname sseq@\sseq@classpattern xoffset#1/#2\endcsname }
    }}
}
...
%
    ...
    \ifnum\sseq@tempx<\sseq@xmaxpp\relax\ifnum\sseq@tempx>\sseq@xminmm\relax\ifnum\sseq@tempy<\sseq@ymaxpp\relax\ifnum\sseq@tempy>\sseq@yminmm\relax
        ...
        \sseq@options@secondpassmode
            ...
            \the\sseq@scope@toks
            \sseq@obj{#1.options}%
        \sseq@eval{\@nx\pgftransformshift{\@nx\pgfpointxy{\the\sseq@tempx + \sseq@xoffset}{\the\sseq@tempy}}} %%%%%
        \sseq@eval{\@nx\pgftransformshift{\@nx\pgfpointxy{-\the\sseq@x}{-\the\sseq@y}}} % someday maybe I should figure 
...
%
    ...
    \fi\fi\fi\fi
    \ifsseq@outofrange
        \sseq@eval{\@nx\pgftransformshift{\@nx\pgfpointxy{\sseq@tempx+\sseq@xoffset}{\sseq@tempy+\sseq@yoffset}}}%
        \sseq@eval{\@nx\pgftransformshift{\@nx\pgfpointxy{-\sseq@x}{-\sseq@y}}}
        \pgftransformresetnontranslations
        \sseq@globalrotatetransform
        \sseq@classplacementtransform
        \sseq@offset{\sseq@thisposnum}{\sseq@obj{partcoord.\sseq@thispos.numnodes}}
...
%
    ...
    #4%
    \sseq@eval{\@nx\pgftransformshift{\@nx\pgfpointxy{-\the\sseq@x}{-\the\sseq@y}}}%
...
\tikzoption{shift}{\tikz@addtransform{\sseq@tikz@scan@one@point@noshift\pgftransformshift#1\relax}}
%\def\tikz@to@compute@relative{%
%  \sseq@tikz@scan@one@point@noshift\tikz@@to@compute@relative(\tikztostart)%
%}
%\def\tikz@@to@compute@relative#1{%
%  \def\tikz@tofrom{#1}%
%  \sseq@tikz@scan@one@point@noshift\tikz@@@to@compute@relative(\tikztotarget)%
%}
%
%\def\tikz@to@compute@distance{\sseq@tikz@scan@one@point@noshift\tikz@@to@compute@distance(\tikz@tofrom)}
%\def\tikz@@to@compute@distance#1{%
%  \def\tikz@first@point{#1}%
%  \sseq@tikz@scan@one@point@noshift\tikz@@@to@compute@distance(\tikz@toto)}
%
\def\tikz@to@curve@path{%
  [every curve to]
  \pgfextra{\iftikz@to@relative\tikz@to@compute@relative\else\tikz@to@compute\fi}%\show\tikztostart
  \tikz@computed@path
  \tikztonodes%
}
