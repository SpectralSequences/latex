...
\def\sseq@setupclip{
    ...
    \ifsseq@clip
        \ifx\sseq@customclip\pgfutil@empty
            ...
            \def\ymax{\pgftransformshift{\pgfqpoint{0pt}{\sseq@yaxis@end@extend+\sseq@clip@padding@top}}}
            \bgroup
            ...
            % This is supposed to go in a \pgftransformshift later that shifts the center of the clipping rectangle to the origin
            % to prepare to use \pgfpointborderrectangle for off page edge handling.
            \edef\sseq@clip@corner{
                \@nx\pgfqpoint{
                    \the\dimexpr
                        \numexpr\sseq@xmax-\sseq@xmin\relax\sseq@xscalecm
                       +\sseq@clip@padding@right
                       +\sseq@yaxisgap
                    \relax
                }{
                    \the\dimexpr
                        \numexpr\sseq@ymax-\sseq@ymin\relax\sseq@yscalecm
                       +\sseq@clip@padding@top
                       +\sseq@xaxisgap
                    \relax
                }
            }
            \sseq@clip@corner
            \edef\sseq@clip@center{
                \@nx\pgfqpoint{
                    \the\dimexpr
                        -\dimexpr
                            \pgf@x + \sseq@yclip@axisgap
                        \relax/2
                       -\sseq@xoffset\sseq@xscalecm
                    \relax
                }{
                    \the\dimexpr
                        -\dimexpr
                            \pgf@y + \sseq@xclip@axisgap
                        \relax/2
                       -\sseq@yoffset\sseq@yscalecm
                    \relax
                }
            }
            % Now we find the offset from the center of the clip to the top right corner.
            \pgf@process{\pgfpointadd{\sseq@clip@center}{\sseq@clip@corner}}
            \edef\sseq@clip@corner{\@nx\pgfqpoint{\the\pgf@x}{\the\pgf@y}}
%
            \bgroup
...
\def\sseq@grid@go{
 ...
 \pgfpathgrid[stepx=\pgf@xa,stepy=\pgf@ya]
    { \pgfqpointxy
        ...
        { \numexpr\sseq@ymin - \sseq@tempy +\sseq@yoffset\relax} }
    { \pgfpointxy
        { \sseq@xmax + \sseq@tempxb - 0.01 +\sseq@xoffset + 1 }
        { \sseq@ymax + \sseq@tempyb - 0.01 +\sseq@yoffset + 1 }
    }
 \pgfusepath{stroke}
 \endpgfscope
 \egroup
