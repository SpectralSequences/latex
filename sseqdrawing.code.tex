...
%
    ...
    %
    \sseq@tempx=\numexpr\sseq@intdivceiling{\sseq@xmin}{2*\sseq@xgridstep}*2*\sseq@xgridstep\relax
...
%    \pgfmathparse{abs(mod(2*\sseq@ygridstep-\sseq@ymin,2*\sseq@ygridstep))}\show\pgfmathresult
    \pgftransformshift{\pgfpointxy{1-\sseq@xminpadding}{1-abs(mod(2*\sseq@ygridstep-\sseq@ymin,2*\sseq@ygridstep))}}

    \pgfpathgrid[stepx= 2cm*\sseq@stepscale,stepy=2cm*\sseq@stepscale]
    { \pgfpointxy { (-\sseq@xgridstep/2 +\sseq@xoffset) } { ( -\sseq@xgridstep/2 + \sseq@yoffset )} }
    { \pgfpointxy { (\sseq@xmax +\sseq@xoffset ) }{ (\sseq@ymax + \sseq@yoffset + 2*\sseq@ygridstep ) } }
    \pgfusepath{stroke}
    \endpgfscope
    \egroup
}
