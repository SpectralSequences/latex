...
% A hook for \DoUntilOutOfBoundsThenNMore
\let\sseq@possible@restorestack\sseq@restorestack
...
%                        }
            ...
            \if_bool:N \l_tmpb_bool\else:
                \sseq@error{DoUntilOutOfBounds-no-bound}
            \fi:
        }
    }
...
%            \fi:
    ...
    \sseq@possible@restorestack % Don't do this if we are inside \DoUntilOutOfBoundsThenNMore
    \endgroup
}

\def \sseq@DoUntilOutOfBoundsThenNMore #1#2 {
    \begingroup
    \let\sseq@possible@restorestack\relax
