...
\cs_new_protected:Npn\sseq@DeclareDocumentCommandAs#1#2#3#4{
    ...
    \cs_if_exist:cTF{ \cs_to_str:N #2 \c_space_tl code }{
        \cs_gset:Npx #1{
            ...
            % Set l__cmd_function_tl so that xparse error messages report the correct command name
            \exp_not:N \tl_set:Nn \exp_not:N \l__cmd_function_tl { \cs_to_str:N #2 }
        ...
        % For commands without a 'code' suffix, we need to wrap the call to set l__cmd_function_tl
        \cs_gset:Npx #1 {
            \exp_not:N \tl_set:Nn \exp_not:N \l__cmd_function_tl { \cs_to_str:N #2 }
            \exp_not:N #2
        }
