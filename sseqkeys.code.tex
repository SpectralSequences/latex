...
\def\sseq@class@positionlabel#1#2{\sseq@pgfkeysdocommand{#1}{#2 of \tikz@fig@name}}
...
\def\sseq@sanitize@intexpr#1#2{
    \sseq@ifintexpr{#1}{
        ...
        \sseq@error@xx{invalid-value-not-intexpr}{\unexpanded{#2}}{#1}
        \sseq@break
    }
}
...
\def\sseq@sanitize@positiveintexpr#1#2{
    ...
    \ifnum\sseq@sanitize@output<1\relax
        \sseq@error@xx{invalid-value-not-positive-intexpr}{\unexpanded{#2}}{#1}
        \sseq@break
    \fi
}
...
\pgfqkeys{/handlers}{
    ...
    .sseq store in sanitize/.code n args={3}{%
        ...
        \pgfkeys{%
            ...
            /sseqpages/global/default/\sseq@temp/.store in/.code/.expanded={%
                \unexpanded{#3{##1}}{\sseq@temp=\unexpanded{##1}}%
...
\sseq@globalkeys{%
    ...
    grid color/.sseq store in=\sseq@gridcolor{gray},
    grid step/.sseq x y alias,
    grid step/.value required,
    x grid step/.sseq store in sanitize=\sseq@xgridstep{1}\sseq@sanitize@positiveintexpr,
    y grid step/.sseq store in sanitize=\sseq@ygridstep{1}\sseq@sanitize@positiveintexpr,
    ...
    class pattern/.add code={\ifcsname sseq@##1 xoffset1/1\endcsname\else\sseq@error{unknown-class-pattern}\@xp\sseq@break\fi}{\sseq@breakpoint},
...
\def\sseq@xrange#1#2{%
    ...
    \sseq@sanitize@intexpr{#1}{x range={#1}{#2}}{#1}
    \sseq@sanitize@intexpr{#2}{x range={#1}{#2}}{#2}
    \xdef\sseq@xmin{\the\numexpr#1}%
    \xdef\sseq@xmax{\the\numexpr#2}%
    \let\xmin\sseq@xmin
    \let\xmax\sseq@xmax
...
%
    ...
    x tick step/.sseq store in sanitize=\sseq@xtickstep{1}\sseq@sanitize@positiveintexpr,
    y tick step/.sseq store in sanitize=\sseq@ytickstep{1}\sseq@sanitize@positiveintexpr,
    tick offset/.sseq x y alias,
    tick offset/.value required,
    x tick offset/.sseq store in sanitize=\sseq@xtickstepoffset{0}\sseq@sanitize@intexpr,
    y tick offset/.sseq store in sanitize=\sseq@ytickstepoffset{0}\sseq@sanitize@intexpr,
...
%
    ...
    x axis origin/.sseq store in sanitize=\sseq@xaxisorigin{0}\sseq@sanitize@intexpr,
    y axis origin/.sseq store in sanitize=\sseq@yaxisorigin{0}\sseq@sanitize@intexpr,
