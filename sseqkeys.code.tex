...
\sseq@addtostorelist\sseq@classname@handler{}
\def\sseq@classlabel@handler@default#1{\def\processedlabel{#1}}
\def\sseq@classname@handler@default#1{\def\processedname{#1}}
...
\pgfqkeys{/sseqpages/class}{
    name/.code={\sseq@classname@handler{#1}\edef\sseq@class@name{\processedname}\pgfkeysalso{/sseqpages/alias=#1}},
...
%
    ...
    class label handler/.code = { % \long so that \ifx\classlabeltextfn\@gobble compares true if this is empty (\@gobble is long...)
        ...
        \ifx\sseq@classlabel@handler\@gobble
            \def\sseq@classlabel@handler##1{\def\processedlabel{##1}}
        \else
    ...
    class name handler/.code = { % \long so that \ifx\classlabeltextfn\@gobble compares true if this is empty (\@gobble is long...)
        \long\def\sseq@classname@handler##1{#1}
        \ifx\sseq@classname@handler\@gobble
            \def\sseq@classname@handler##1{\def\processedname{##1}}
        \else
    ...
    class name handler/.sseq default code={
        ...
        \ifx\sseq@classname@handler@default\@gobble
            \def\sseq@classname@handler@default##1{\def\processedlabel{##1}}
        \else
