...
\def\sseq@keys@addtooptions@checkalso@#1\pgfkeysalso#2{%
    ...
    \else
        \ifx\expandafter#2 % if they wanted to expand the first token, handle that
            \sseq@add@tikzkey@to@options{\relax\relax} % The last token was an \expandafter, we need to keep it from doing anything
...
\def\sseq@pgfkeys@case@one@store{%
    \pgfkeysifdefined{\pgfkeyscurrentkey/.@cmd}{%
        ...
        \sseq@eval{\@nx\sseq@keys@addtooptions@checkalso{\@nx\sseq@options@secondpass{\unexpanded\@xptwo{\@xp\pgfkeys@code\pgfkeyscurrentvalue\pgfeov}}}}%
    }{\pgfkeys@case@two}%
}
...
%
    x axis type/.is choice,
    y axis type/.is choice,
    axes type/.sseq x y alias=axis style,
    ~x axis type/border/.code={\sseq@drawxaxistrue\sseq@borderxaxistrue},
    ~y axis type/border/.code={\sseq@drawyaxistrue\sseq@borderyaxistrue},
    ~x axis type/center/.code={
        \sseq@drawxaxistrue\sseq@borderxaxisfalse
        \ifx\sseq@xaxisgap\sseq@xaxisgap@default
            \def\sseq@xaxisgap{0cm}
        \fi
    },
    ~y axis type/center/.code={\sseq@drawyaxistrue\sseq@borderyaxisfalse\ifx\sseq@yaxisgap\sseq@yaxisgap@default\def\sseq@yaxisgap{0cm}\fi},
    ~x axis type/none/.code={\sseq@drawxaxisfalse},
    ~y axis type/none/.code={\sseq@drawyaxisfalse},
    x axis origin/.sseq store int in=\sseq@xaxisorigin{0},
    y axis origin/.sseq store int in=\sseq@yaxisorigin{0},
