...
% Not
\def\code@catcode@hook{\preventhypenligatures\catcode`"=\active}
%\begingroup\lccode`~=`>
%  \lowercase{\endgroup\gdef~}{>}

\begingroup\lccode`~=`-
  \lowercase{\endgroup\gdef~}{-}
\def\preventhypenligatures{%
  \begingroup\lccode`~=`-
  \lowercase{\endgroup\def~}{-\kern0pt }%
%  \begingroup\lccode`~=`>
%  \lowercase{\endgroup\def~}{>{}}%
%  \catcode`\>=\active
}

% For some reason without this " appears as [. No idea why.
\begingroup\lccode`~=`"
  \lowercase{\endgroup\gdef~}{"}

% Maybe there's a better way to do this? Have to use \scantokens to apply active catcode to dash.
%\def\code@catcode@hook{\preventhypenligatures}
%\pgfqkeys{/codeexample}{typeset listing/.prefix code={\preventhypenligatures}}
