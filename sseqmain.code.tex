...
%
    % To use old values, need the scale, target width, target height, xmin, xmax, ymin, ymax, and backgroundpaths to be the same
    \edef\sseq@scalehash{\sseq@xscale;\sseq@yscale;\sseq@fitwidth;\sseq@fitheight;\sseq@xmin;\sseq@xmax;\sseq@ymin;\sseq@ymax}
    \sseq@tempiffalse
    \sseq@obj@ifdef{scalehash.\sseq@scalehash}{
        \@xp\ifx\csname\sseq@obj@fullname{scalehash.\sseq@scalehash}\endcsname\sseq@savedbackgroundpaths
            \sseq@tempiftrue 
        \fi
    }{}
    \ifsseq@tempif % Cache hit
        \sseq@lettoobj\sseq@xscale{scalehash.\sseq@scalehash.xscale}
        \sseq@lettoobj\sseq@yscale{scalehash.\sseq@scalehash.yscale}
        \sseq@xscalecm = \sseq@xscale cm
        \sseq@yscalecm = \sseq@yscale cm
    \else % Otherwise
        \sseq@checkrangeandfindscale % calculate scale, check for error
        % cache result
        \sseq@obj@xdef{scalehash.\sseq@scalehash.xscale}{\sseq@xscale}
        \sseq@obj@xdef{scalehash.\sseq@scalehash.yscale}{\sseq@yscale}
        \sseq@obj@xodef{scalehash.\sseq@scalehash}{\sseq@savedbackgroundpaths}
    \fi
%
...
\newbox\sseq@measureheightwidthbox
\def\sseq@checkrangeandfindscale{
...
%
    ...
    \fi
%
    \sseq@globalscaleoption{\pgfsetxvec{\pgfpoint{\sseq@xscalecm}{0cm}}\pgfsetyvec{\pgfpoint{0cm}{\sseq@yscalecm}}}
    \let\sseq@xscale@save\sseq@xscale
    \let\sseq@yscale@save\sseq@yscale
%
    \def\sseq@fittingiterations{1}
%
    \ifx\sseq@fitwidth\pgfutil@empty % Check to make sure chart will fit on page
        \edef\sseq@findscalewidth{\the\textwidth}
    \else % scale to fit target width
        \ifnum\sseq@xmax=\sseq@xmin
            \sseq@error@n{height-width-range-zero}{width}{x}
            \let\sseq@fitwidth\pgfutil@empty
        \else
            \let\sseq@findscalewidth\sseq@fitwidth
            \ifsseq@backgroundpaths
                \def\sseq@fittingiterations{3}
            \fi
        \fi
    \fi
%
    \ifx\sseq@fitheight\pgfutil@empty % Check to make sure chart will fit on page
        \edef\sseq@findscaleheight{\the\textheight}
    \else % Set yscale to fit target height
        \ifnum\sseq@ymax=\sseq@ymin
            \sseq@error@n{height-width-range-zero}{height}{y}
            \let\sseq@fitheight\pgfutil@empty
        \else
            \let\sseq@findscaleheight\sseq@fitheight
            \ifsseq@backgroundpaths
                \def\sseq@fittingiterations{3}
            \fi
        \fi
    ...
    \sseq@findscale
%
    \ifx\sseq@fitwidth\pgfutil@empty
        \ifdim\dimexpr\sseq@scaledwidth>\textwidth
          \sseq@error@xxxx{range-overflow}{x}
                {from \sseq@xmin\space to \sseq@xmax\space (xmax - xmin = \the\numexpr\sseq@xmax-\sseq@xmin)}
                {\sseq@maxxrange}% max x range
                {\sseq@xscale}% max xscale
            \@xp\sseq@break
        \fi
                %\sseq@tempdimen = \dimexpr \textheight - \sseq@totalymargin \relax
        \let\sseq@xscale\sseq@xscale@save
        \sseq@xscalecm = \sseq@xscale cm
    \fi
%
    \ifx\sseq@fitheight\pgfutil@empty
        \ifdim\dimexpr\sseq@scaledheight>\textheight
            \pgfmathparse{\sseq@tempdimen/1cm/(\sseq@ymax-\sseq@ymin+1)}
            \sseq@error@xxxx{range-overflow}{y}
                {from \sseq@ymin\space to \sseq@ymax\space (ymax - ymin = \the\numexpr\sseq@ymax-\sseq@ymin)}
                {\sseq@maxyrange}% max y range
                {\sseq@yscale}% max yscale
            \@xp\sseq@break
        \fi
        \let\sseq@yscale\sseq@yscale@save
        \sseq@yscalecm = \sseq@yscale cm
    \fi
}
...
\def\sseq@findscale{
    \sseq@findscale@step
    \pgfmathtruncatemacro\sseq@maxxrange{(\textwidth -\sseq@totalxmargin)/(\sseq@xscale@save cm)}
    \pgfmathtruncatemacro\sseq@maxyrange{(\textheight -\sseq@totalymargin)/(\sseq@yscale@save cm)}
    \edef\sseq@scaledwidth{\the\wd\sseq@measureheightwidthbox}
    \edef\sseq@scaledheight{\the\ht\sseq@measureheightwidthbox}
    \prg_replicate:nn{\sseq@fittingiterations}{\sseq@findscale@step}
}
\ExplSyntaxOff

\def\sseq@findscale@step{
    \setbox\sseq@measureheightwidthbox=\hbox{%
        \begin{sseq@tikzpicture}%
        \end{sseq@tikzpicture}%
    }
    \edef\sseq@totalxmargin{\the\dimexpr\wd\sseq@measureheightwidthbox - \sseq@xscalecm*(\sseq@xmax-\sseq@xmin)}
    \edef\sseq@totalymargin{\the\dimexpr\ht\sseq@measureheightwidthbox - \sseq@yscalecm*(\sseq@ymax-\sseq@ymin)}
%
    \ifnum\sseq@xmax>\sseq@xmin
        \sseq@tempdimen = \dimexpr \sseq@findscalewidth - \sseq@totalxmargin\relax
        \sseq@xscalecm = \dimexpr\sseq@tempdimen/(\sseq@xmax-\sseq@xmin)\relax
        \pgfmathsetmacro\sseq@xscale{\sseq@xscalecm /1cm}
    \fi
%
    \ifnum\sseq@ymax>\sseq@ymin
        \sseq@tempdimen = \dimexpr \sseq@findscaleheight - \sseq@totalymargin\relax
        \sseq@yscalecm = \dimexpr\sseq@tempdimen/(\sseq@ymax-\sseq@ymin)\relax
        \pgfmathsetmacro\sseq@yscale{\sseq@yscalecm /1cm}
    \fi
}



\def\sseq@tikzpicture{
...
\def\endsseq@tikzpicture{\end{tikzpicture}}
...
\def\sseq@qcolor#1{\unexpanded{\sseq@options@secondpass{\tikz@addoption{\tikz@compat@color@set{#1}}\def\tikz@textcolor{#1}}}}

\def\sseq@qclass{\@ifnextchar[{\sseq@qclass@}{\sseq@qclass@[]}}
\def\sseq@qclass@[#1](#2,#3){
    \sseq@obj@inccheckdef{partcoord.(#2,#3).numnodes}
    \edef\tempn{\sseq@obj{partcoord.(#2,#3).numnodes}}
    \edef\classname{class.(#2,#3,\tempn)}
    \sseq@obj@xdef{\classname.n}{\tempn}
    \sseq@obj@xdef{\classname.num}{0}
    \sseq@obj@xdef{\classname[0].page}{\sseq@infinity}
    \sseq@obj@xdef{\classname[0].options}{#1}
    \sseq@savedpaths@addclass{class.(#2,#3,\tempn)}
}

\def\sseq@qclassnamed{\@ifnextchar[{\sseq@qclassnamed@}{\sseq@qclassnamed@[]}}
\def\sseq@qclassnamed@[#1](#2,#3)#4{
    \sseq@qclass@[#1](#2,#3)
    \def\sseq@class@name{#4}
    \sseq@obj@xdef{\classname[0].name}{\sseq@class@name}
    \sseq@obj@xdef{class.namedclass.\detokenize\@xp{\sseq@class@name}}{#2,#3,\tempn}
}
...
\def\sseq@class@copy#1#2{
    ...
    \edef\sseq@class@tag{copiedfrom\sseq@thisposnodeindex}
...
% Exposed:
\def\sseq@lastlabel{\sseq@lastlabel@}
\let\sseq@lastlabel@\empty
