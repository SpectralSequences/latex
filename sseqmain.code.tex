...
%    
    ...
    \else
        \ifnum\sseq@classinsert<0\relax
            % negative 1 is where we would normally insert things
            \edef\sseq@classinsert{\the\numexpr\nodenum+\sseq@classinsert+1}
        \fi
        \ifodd0% if( n < 1 or n > \nodenum ) 
            \ifnum\sseq@classinsert<\z@ 1\fi
            \ifnum\sseq@classinsert>\numexpr\nodenum\relax 1\fi
        \relax
            \sseq@error{class-insert-out-of-range}
            \sseq@break
%%%% No more breaks after this point -- if we need them later for some reason, have to refactor this.
        \else 
