...
%
    ...
    \ifnum \numexpr\sseq@xmax-\sseq@xmin > \numexpr\maxdimen/\dimexpr\sseq@xscale\relax\relax
        \pgfmathparse{\maxdimen/1cm/(\sseq@xmax-\sseq@xmin)}
        \sseq@error@xxxx{range-overflow}{x}
            {from \sseq@xmin\space to \sseq@xmax\space (xmax - xmin = \the\numexpr\sseq@xmax-\sseq@xmin)}
            {\the\numexpr\maxdimen/\dimexpr\sseq@xscale\relax-1\relax}% max x range
            {\pgfmathresult}% max xscale
        \@xp\sseq@break
    \fi

    \ifnum \numexpr\sseq@ymax-\sseq@ymin > \numexpr\maxdimen/\dimexpr\sseq@xscale\relax\relax
        \pgfmathparse{\maxdimen/1cm/(\sseq@ymax-\sseq@ymin)}
        \sseq@error@xxxx{range-overflow}{y}
            {from \sseq@ymin\space to \sseq@ymax\space (ymax - ymin = \the\numexpr\sseq@ymax-\sseq@ymin)}
            {\the\numexpr\maxdimen/\dimexpr\sseq@yscale\relax-1\relax}% max x range
            {\pgfmathresult}% max xscale
        \@xp\sseq@break
    \fi

%    %\showthe\numexpr\textwidth/\dimexpr\sseq@xscale\relax
%    %\showthe\numexpr\sseq@xmax-\sseq@xmin
%    \showthe\textwidth
%    \show\sseq@xscale
%    \showthe\numexpr\textwidth/\dimexpr\sseq@xscale\relax\relax
%    \ifnum \numexpr\sseq@xmax-\sseq@xmin - \numexpr\textwidth/\dimexpr\sseq@xscale\relax\relax>0\relax
%        \hi
%        \pgfmathparse{\textwidth/1cm/(\sseq@xmax-\sseq@xmin)}
%        \sseq@error@xxxx{range-overflow}{x}
%            {from \sseq@xmin\space to \sseq@xmax\space (xmax - xmin = \the\numexpr\sseq@xmax-\sseq@xmin)}
%            {\the\numexpr\textwidth/\dimexpr\sseq@xscale\relax-1\relax}% max x range
%            {\pgfmathresult}% max xscale
%        \@xp\sseq@break
%    \fi
...
\ifsseq@draftmode % Okay, have to dummy out all the main commands.
    ...
    \sseq@DeclareDocumentCommand\structline{od()d()}{} % TODO: make these absorb second optional argument if there's a space (source) (target)
